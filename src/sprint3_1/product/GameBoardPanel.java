        ...
        for (int row = 0; row < gameLogic.getTotalRows(); ++row) {
            for (int col = 0; col < gameLogic.getTotalColumns(); ++col) {
                if (gameLogic.getCell(row, col) == GameLogic.Cell.O) {
                    ...
                    g.drawString("O", x, y);
                    if(gameLogic.getCombinationDirection(row, col) != GameLogic.CombinationDirection.EMPTY){
                        //maybe I need to use a queue or something
                        //col + sin (index * 45)
//                        (int) Math.round(Math.cos(Math.toRadians(45 * i)))
//                        Season.valueOf("WINTER").ordinal())
//                        System.out.println(CombinationDirection.N.ordinal());

                        //i replaced by ordinal
//                        int x1 = (col + (int) Math.round(-1 * Math.cos(Math.toRadians(45 * gameLogic.getCombinationDirection(row, col).ordinal())))) * cellSize + cellPadding;
//                        int y1 = (row + (int) Math.round(-1 * Math.sin(Math.toRadians(45 * gameLogic.getCombinationDirection(row, col).ordinal())))) * cellSize + cellPadding;
////                        int x2 = col + (int) Math.round(Math.cos(Math.toRadians(45 * gameLogic.getCombinationDirection(row, col).ordinal()))) * cellSize + cellPadding;
////                        int y2 = row + (int) Math.round(Math.sin(Math.toRadians(45 * gameLogic.getCombinationDirection(row, col).ordinal()))) * cellSize + cellPadding;
//                        int x2 = (col + (int) Math.round(Math.cos(Math.toRadians(45 * gameLogic.getCombinationDirection(row, col).ordinal())))) * cellSize + cellPadding;
//                        int y2 = (row + (int) Math.round(Math.sin(Math.toRadians(45 * gameLogic.getCombinationDirection(row, col).ordinal())))) * cellSize + cellPadding;
                        //I start a for loop here
                        //have a list of queues
                        //then instead of cell check if the inputed list is empty
                        int x1 = (col + (int) Math.round(-1 * Math.cos(Math.toRadians(45 * gameLogic.getCombinationDirection(row, col).ordinal())))) * cellSize + (cellSize / 2);
                        int y1 = (row + (int) Math.round(-1 * Math.sin(Math.toRadians(45 * gameLogic.getCombinationDirection(row, col).ordinal())))) * cellSize + (cellSize / 2);
//                        int x2 = col + (int) Math.round(Math.cos(Math.toRadians(45 * gameLogic.getCombinationDirection(row, col).ordinal()))) * cellSize + cellPadding;
//                        int y2 = row + (int) Math.round(Math.sin(Math.toRadians(45 * gameLogic.getCombinationDirection(row, col).ordinal()))) * cellSize + cellPadding;
                        int x2 = (col + (int) Math.round(Math.cos(Math.toRadians(45 * gameLogic.getCombinationDirection(row, col).ordinal())))) * cellSize + (cellSize / 2);
                        int y2 = (row + (int) Math.round(Math.sin(Math.toRadians(45 * gameLogic.getCombinationDirection(row, col).ordinal())))) * cellSize + (cellSize / 2);
                    ...
                    if(gameLogic.getCombinationDirection(row, col) != GameLogic.CombinationDirection.EMPTY){
                        ...
                        int x2 = (col + 2 * (int) Math.round(Math.cos(Math.toRadians(45 * gameLogic.getCombinationDirection(row, col).ordinal())))) * cellSize + (cellSize / 2);
                        int y2 = (row + 2 * (int) Math.round(Math.sin(Math.toRadians(45 * gameLogic.getCombinationDirection(row, col).ordinal())))) * cellSize + (cellSize / 2);
                        System.out.println(x1 + " " + y1 + " " + x2 + " " + y2);
                        g2d.setColor(Color.RED);
                        g2d.drawLine(x1, y1, x2, y2);
                        g2d.setColor(Color.BLACK);
